...
test8::test8(const char* config_name, const char* sheet_name)
{
...
void test8::afterInit()
{
}

void test8::beforeUpdate()
{
		...
		//因此我们就不多此一举给每个要素设计独立的时间了、而是做一下SWMM要素与LISFLOOD中的要素的时间匹配
	{
		...
		for (int i = 0; i < size; i++) p_val[i] = 0;
		...
		for (river_it = river_pipe_index.begin(); river_it != river_pipe_index.end(); ++river_it)
		{
			...
			if (temp != pipe_upstreamlink_index.end()) {
				...
				config_pipe->setOutletDepth(t.node, ( H+ elevation-1.84));
...
void test8::afterUpdate()
{
	//do something
}

void test8::beforeFinalize()
{
	//do something
}
