...
! --- Huang change -- new advection scheme 3
!        I1=NTRG(I)
!        FIJ1=(dye(nv(I1,1),k)+dye(nv(I1,2),k)+dye(nv(I1,3),k))/3.0_SP
!        FIJ2=FIJ1
!       if ((FIJ1<-0.001) .or. (FIJ1>30.001)) print *,FIJ1,IA,k,iint,'FIJ1 new scheme'
! --- Huang change -- new advection scheme 2
!       FIJ1=dye(IA,K)
!       FIJ2=dye(IB,K)
! --- Huang change -- new advection scheme 1
...
!       if ((FIJ1<-0.001) .or. (FIJ1>30.001)) print *,FIJ1,IA,k,iint,'FIJ1 new scheme'
!       if ((FIJ2<-0.001) .or. (FIJ2>30.001)) print *,FIJ2,IA,k,iint,'FIJ2 new scheme'
! Huang change end
       UN=UVN(I,K)
   ...
   DO K=1,KBM1
     DO I=1,M
       ...
       IF(ISWETN(I)*ISWETNT(I) == 1) THEN
        ...
        ZZZFLUX(I,K)= TEMP*(DTI/DT(I))/DZ(K) + XFLUX(I,K)/ART1(I)*(DTI/DT(I)) 
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   DO ITERA=1,NTERA   !! Smolaricizw Loop 
     ...
     DO K=2,KBM1
       DO I=1,M
         TEMP=ABS(WWWSF(I,K))-DTI*(WWWSF(I,K))*(WWWSF(I,K))/DZ(K)/DTWWWS(I)
     ...
     DO I=1,M
       ...
       TEMP=0.5*((WWWS(I,2)+ABS(WWWS(I,2)))*DYE_S(I,2))*(DTI/DTFA(I))/DZ(1)
       BETAIN(I,1)=(SMAX-DYE_S(I,1))/(TEMP+1.E-10)

       TEMP=0.5*((WWWS(I,1)+ABS(WWWS(I,1)))*DYE_S(I,1)-    &
            (WWWS(I,2)-ABS(WWWS(I,2)))*DYE_S(I,1))*(DTI/DTFA(I))/DZ(1)
     ...
     DO K=2,KBM1-1
       DO I=1,M
         ...
         TEMP=0.5*((WWWS(I,K+1)+ABS(WWWS(I,K+1)))*DYE_S(I,K+1)-   &
	      (WWWS(I,K)-ABS(WWWS(I,K)))*DYE_S(I,K-1))*(DTI/DTFA(I))/DZ(K)
         BETAIN(I,K)=(SMAX-DYE_S(I,K))/(TEMP+1.E-10)

         TEMP=0.5*((WWWS(I,K)+ABS(WWWS(I,K)))*DYE_S(I,K)-         &
	      (WWWS(I,K+1)-ABS(WWWS(I,K+1)))*DYE_S(I,K))*(DTI/DTFA(I))/DZ(K)
     ...
     DO K=1,KBM1
       DO I=1,M
         ...
         IF(ISWETN(I)*ISWETNT(I) == 1) THEN
           ...
           END IF
           TEMP = 0.5_SP*TEMP
           DYE_SF(I,K)=(   DYE_S(I,K)-TEMP*(DTI/DTFA(I))/DZ(K)   ) 
...
!#  if defined (MPDATA)
       DYEF(I,K)=DYE_SF(I,K)
!#  else     
!     DYEF(I,K)=(DYE(I,K)-XFLUX(I,K)/ART1(I)*(DTI/DT(I)))*(DT(I)/DTFA(I))
!#  endif
     END DO
...
!==============================================================================|
   ...
   DO I=1,M
     ...
     DO K=1,KBM1
       ...
       IF(K == 1)THEN
         SMAX = MAX(SMAX,(DYE(I,K)*DZ(K+1)+DYE(I,K+1)*DZ(K))/(DZ(K)+DZ(K+1)))
         SMIN = MIN(SMIN,(DYE(I,K)*DZ(K+1)+DYE(I,K+1)*DZ(K))/(DZ(K)+DZ(K+1)))
       ELSE IF(K == KBM1)THEN
         SMAX = MAX(SMAX,(DYE(I,K)*DZ(K-1)+DYE(I,K-1)*DZ(K))/(DZ(K)+DZ(K-1)))
         SMIN = MIN(SMIN,(DYE(I,K)*DZ(K-1)+DYE(I,K-1)*DZ(K))/(DZ(K)+DZ(K-1)))
       ELSE
         SMAX = MAX(SMAX,(DYE(I,K)*DZ(K-1)+DYE(I,K-1)*DZ(K))/(DZ(K)+DZ(K-1)), &
                 (DYE(I,K)*DZ(K+1)+DYE(I,K+1)*DZ(K))/(DZ(K)+DZ(K+1)))
         SMIN = MIN(SMIN,(DYE(I,K)*DZ(K-1)+DYE(I,K-1)*DZ(K))/(DZ(K)+DZ(K-1)), &
                 (DYE(I,K)*DZ(K+1)+DYE(I,K+1)*DZ(K))/(DZ(K)+DZ(K+1)))
       END IF
...
!
     DO I=1,IOBCN
       ...
       XFLUX2D=0.0_SP
       DO K=1,KBM1
         S2D=S2D+DYE(J,K)*DZ(K)
         S2D_NEXT=S2D_NEXT+DYEF(J1,K)*DZ(K)
       ...
       IF(UARD_OBCN(I) > 0.0_SP) THEN
         ...
         DO K=1,KBM1
!           DYEF(J,K)=S2D_OBC+(DYEF(J1,K)-S2D_NEXT)
           DYEF(J,K)=DYEF(J1,K)
          END DO
         ...
         DO K=1,KBM1
           ...
           IF(K == 1)THEN
            SMAX = MAX(SMAX,(DYE(J,K)*DZ(K+1)+DYE(J,K+1)*DZ(K))/  &
                   (DZ(K)+DZ(K+1)))
            SMIN = MIN(SMIN,(DYE(J,K)*DZ(K+1)+DYE(J,K+1)*DZ(K))/  &
                   (DZ(K)+DZ(K+1)))
           ELSE IF(K == KBM1)THEN
            SMAX = MAX(SMAX,(DYE(J,K)*DZ(K-1)+DYE(J,K-1)*DZ(K))/  &
                   (DZ(K)+DZ(K-1)))
