   ...
   DO K=1,KBM1
     DO I=1,M
       IF(ISWETN(I)*ISWETNT(I) == 1) THEN
         ...
         ZZZFLUX(I,K) = TEMP*(DTI/DT(I))/DZ(K) + XFLUX(I,K,N1)/ART1(I)*(DTI/DT(I)) 
...
!----------------------------------------------------------------------------------------
   ...
   DO ITERA=1,NTERA   !! Smolaricizw Loop 
     ...
     DO I=1,M
       ...
       TEMP=0.5*((WWWS(I,2)+ABS(WWWS(I,2)))*WQM_S(I,2))*(DTI/DTFA(I))/DZ(1)
       BETAIN(I,1)=(WQMMAX-WQM_S(I,1))/(TEMP+1.E-10)

       TEMP=0.5*((WWWS(I,1)+ABS(WWWS(I,1)))*WQM_S(I,1)-        &
	           (WWWS(I,2)-ABS(WWWS(I,2)))*WQM_S(I,1))*(DTI/DTFA(I))/DZ(1)
     ...
     DO K=2,KBM1-1
       DO I=1,M
         ...
         TEMP=0.5*((WWWS(I,K+1)+ABS(WWWS(I,K+1)))*WQM_S(I,K+1)-  &
	           (WWWS(I,K)-ABS(WWWS(I,K)))*WQM_S(I,K-1))*(DTI/DTFA(I))/DZ(K)
         BETAIN(I,K)=(WQMMAX-WQM_S(I,K))/(TEMP+1.E-10)

         TEMP=0.5*((WWWS(I,K)+ABS(WWWS(I,K)))*WQM_S(I,K)-        &
	           (WWWS(I,K+1)-ABS(WWWS(I,K+1)))*WQM_S(I,K))*(DTI/DTFA(I))/DZ(K)
     ...
     DO K=1,KBM1
       DO I=1,M
         ...
         IF(WET_DRY==1.AND.ISWETN(I)*ISWETNT(I) == 1) THEN
           ...
           END IF
           TEMP = 0.5_SP*TEMP
           WQM_SF(I,K)=(WQM_S(I,K)-TEMP*(DTI/DTFA(I))/DZ(K)) 
       endif
	   END DO
     END DO  !! SIGMA LOOP
...
!
     DO K=1,KBM1
       ...
       IF(WET_DRY==1.AND.ISWETN(I)*ISWETNT(I) == 1) THEN
         ...
         IF(K == 1) THEN
           TEMP=-WTS(I,K+1)*(WQM(I,K,N1)*DZ(K+1)+WQM(I,K+1,N1)*DZ(K))/(DZ(K)+DZ(K+1))
         ELSE IF(K == KBM1) THEN
           TEMP=WTS(I,K)*(WQM(I,K,N1)*DZ(K-1)+WQM(I,K-1,N1)*DZ(K))/(DZ(K)+DZ(K-1))
         ELSE
           TEMP=WTS(I,K)*(WQM(I,K,N1)*DZ(K-1)+WQM(I,K-1,N1)*DZ(K))/(DZ(K)+DZ(K-1))-  &
                WTS(I,K+1)*(WQM(I,K,N1)*DZ(K+1)+WQM(I,K+1,N1)*DZ(K))/(DZ(K)+DZ(K+1))
         END IF
...
!
     DO N1=1,NB
       DO I=1,IOBCN
         ...
         XFLUX2D=0.0_SP
         DO K=1,KBM1
           T2D=T2D+WQM(J,K,N1)*DZ(K)
           T2D_NEXT=T2D_NEXT+WQM_F(J1,K,N1)*DZ(K)
...
!qxu{	 
         DO K=1,KBM1
           ...
           IF(K == 1)THEN
            WQMMAX = MAX(WQMMAX,(WQM(J,K,N1)*DZ(K+1)+WQM(J,K+1,N1)*DZ(K))/(DZ(K)+DZ(K+1)))
            WQMMIN = MIN(WQMMIN,(WQM(J,K,N1)*DZ(K+1)+WQM(J,K+1,N1)*DZ(K))/(DZ(K)+DZ(K+1)))
           ELSE IF(K == KBM1)THEN
            WQMMAX = MAX(WQMMAX,(WQM(J,K,N1)*DZ(K-1)+WQM(J,K-1,N1)*DZ(K))/(DZ(K)+DZ(K-1)))
            WQMMIN = MIN(WQMMIN,(WQM(J,K,N1)*DZ(K-1)+WQM(J,K-1,N1)*DZ(K))/(DZ(K)+DZ(K-1)))
           ELSE
            WQMMAX = MAX(WQMMAX,(WQM(J,K,N1)*DZ(K-1)+WQM(J,K-1,N1)*DZ(K))/(DZ(K)+DZ(K-1)), &
                    (WQM(J,K,N1)*DZ(K+1)+WQM(J,K+1,N1)*DZ(K))/(DZ(K)+DZ(K+1)))
            WQMMIN = MIN(WQMMIN,(WQM(J,K,N1)*DZ(K-1)+WQM(J,K-1,N1)*DZ(K))/(DZ(K)+DZ(K-1)), &
                    (WQM(J,K,N1)*DZ(K+1)+WQM(J,K+1,N1)*DZ(K))/(DZ(K)+DZ(K+1)))
           END IF
...
!----------------------------------------------- 
   ...
   DO N1=1,NB
     ...
     IF(ISWETN(I) == 1)THEN
         ...
         VHPF(I,1,N1) = -DTI * WFSURF(I,N1) / (-DZ(1)*D(I)) - F(I,1,N1)
     ...
     DO N1 = 1, NB
       DO K = 1, KBM1
         DO I = 1, M
           ...
           IF(ISWETN(I)*ISWETNT(I) == 1 ) F(I,KB,N1) = F(I,KBM1,N1)-DZZ(KBM1)*D(I)*      &
