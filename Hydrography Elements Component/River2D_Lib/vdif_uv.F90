...
!------------------------------------------------------------------------------|
   ...
   DO K = 2, KBM1
     DO I = 1, N
       IF(ISWETC(I) == 1)THEN
         ...
         A(I,K-1) = -DTI *(C(I,K)+UMOL) / (DZ(K-1)*DZZ(K-1)* D1(I)*D1(I))
         C(I,K) = -DTI *(C(I,K)+UMOL) / (DZ(K)*DZZ(K-1)* D1(I)*D1(I))
       END IF
     END DO
   END DO
   ...
   DO I = 1, N
       ...
       VHPU(I,1) = (-DTI*WUSURF(I)*TMP/(-DZ(1)*D1(I))-UF(I,1)) / (A(I,1)-1.)
       VHPV(I,1) = (-DTI*WVSURF(I)*TMP/(-DZ(1)*D1(I))-VF(I,1)) / (A(I,1)-1.)
     END IF
   END DO
   ...
   DO  I = 1, N
       ...
       (TPS(I)*DTI/(-DZ(KBM1)*D1(I))-1.-(VHU(I,KBM2)-1.)*C(I,KBM1))
       VF(I,KBM1) = (C(I,KBM1)*VHPV(I,KBM2)-VF(I,KBM1))/ &
       (TPS(I)*DTI/(-DZ(KBM1)*D1(I))-1.-(VHV(I,KBM2)-1.)*C(I,KBM1))
     END IF
   END DO
