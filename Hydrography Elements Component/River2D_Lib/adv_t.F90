...
!       VISCOFF(I)=SQRT(TMP1+TMP2)*ART1(I)
    ...
    if  (SPHERICAL==1)then
    if  (NORTHPOLE==1)then
     CALL ADV_T_XY(XFLUX,XFLUX_ADV,PTPX,PTPY,PTPXD,PTPYD,VISCOFF,K)
    endif
    endif  
       ...
       IF(NUMQBC > 1) THEN
         DO J=1,NUMQBC
           ...
           DO K=1,KBM1
             STPOINT1 = T1(J1,K)
             STPOINT2 = T1(J2,K)
             XFLUX(J1,K)=XFLUX(J1,K)-QDIS(J)*RDISQ(J,1)*VQDIST(J,K)*STPOINT1/DZ(K)
             XFLUX(J2,K)=XFLUX(J2,K)-QDIS(J)*RDISQ(J,2)*VQDIST(J,K)*STPOINT2/DZ(K)
           END DO
         END DO
       END IF
     END IF
...
!----------------------------------------------------------------------------------------
   ...
   DO ITERA=1,NTERA   !! Smolaricizw Loop 
     ...
     DO I=1,M
       ...
       TEMP=0.5*((WWWS(I,2)+ABS(WWWS(I,2)))*T1_S(I,2))*(DTI/DTFA(I))/DZ(1)
       BETAIN(I,1)=(TMAX-T1_S(I,1))/(TEMP+1.E-10)

       TEMP=0.5*((WWWS(I,1)+ABS(WWWS(I,1)))*T1_S(I,1)-        &
	           (WWWS(I,2)-ABS(WWWS(I,2)))*T1_S(I,1))*(DTI/DTFA(I))/DZ(1)
     ...
     DO K=2,KBM1-1
       DO I=1,M
         ...
         TEMP=0.5*((WWWS(I,K+1)+ABS(WWWS(I,K+1)))*T1_S(I,K+1)-  &
	           (WWWS(I,K)-ABS(WWWS(I,K)))*T1_S(I,K-1))*(DTI/DTFA(I))/DZ(K)
         BETAIN(I,K)=(TMAX-T1_S(I,K))/(TEMP+1.E-10)

         TEMP=0.5*((WWWS(I,K)+ABS(WWWS(I,K)))*T1_S(I,K)-        &
	           (WWWS(I,K+1)-ABS(WWWS(I,K+1)))*T1_S(I,K))*(DTI/DTFA(I))/DZ(K)
     ...
     DO K=1,KBM1
       DO I=1,M
         IF(ISWETN(I)*ISWETNT(I) == 1) THEN
           ...
           END IF
           TEMP = 0.5_SP*TEMP
           T1_SF(I,K)=(T1_S(I,K)-TEMP*(DTI/DTFA(I))/DZ(K)) 
         END IF
       END DO
     END DO  !! SIGMA LOOP
...
!--------------------------------------------------------------------
   DO K=1,KBM1
     DO I=1,M
       IF(ISWETN(I)*ISWETNT(I) == 1) THEN
       IF(K == 1) THEN
         TEMP=-WTS(I,K+1)*(T1(I,K)*DZ(K+1)+T1(I,K+1)*DZ(K))/(DZ(K)+DZ(K+1))
       ELSE IF(K == KBM1) THEN
         TEMP= WTS(I,K)*(T1(I,K)*DZ(K-1)+T1(I,K-1)*DZ(K))/(DZ(K)+DZ(K-1))
       ELSE
         TEMP= WTS(I,K)*(T1(I,K)*DZ(K-1)+T1(I,K-1)*DZ(K))/(DZ(K)+DZ(K-1))-&
               WTS(I,K+1)*(T1(I,K)*DZ(K+1)+T1(I,K+1)*DZ(K))/(DZ(K)+DZ(K+1))
       END IF
...
!
   DO I=1,M
     IF(ISWETN(I)*ISWETNT(I) == 1 )THEN
     DO K=1,KBM1
       XFLUX(I,K) = XFLUX(I,K) - RF(I,K)*ART1(I)/DZ(K)
    if (MPDATA==0) then    
       TF1(I,K)=(T1(I,K)-XFLUX(I,K)/ART1(I)*(DTI/DT(I)))*(DT(I)/DTFA(I))
    else if(MPDATA==1)then
       TF1(I,K)=T1_SF(I,K) 
!	TF1(I,K)=T1_SF(I,K) - RF(I,K)*DTI/DTFA(I)/DZ(K)  ! considering about heat flux source term, not tested yet
   endif              
     END DO
     ELSE
     DO K=1,KBM1
