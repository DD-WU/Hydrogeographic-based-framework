...
!       VISCOFF(I)=SQRT(TMP1+TMP2)*ART1(I)
    ...
    if (SPHERICAL==1)then
    if  (NORTHPOLE==1)then
     CALL ADV_S_XY(XFLUX,XFLUX_ADV,PSPX,PSPY,PSPXD,PSPYD,VISCOFF,K)
    endif
    endif  
...
!----------------------------------------------------------------------------------------
   ...
   DO ITERA=1,NTERA   !! Smolaricizw Loop 
     ...
     DO I=1,M
       ...
       TEMP=0.5*((WWWS(I,2)+ABS(WWWS(I,2)))*S1_S(I,2))*(DTI/DTFA(I))/DZ(1)
       BETAIN(I,1)=(SMAX-S1_S(I,1))/(TEMP+1.E-10)

       TEMP=0.5*((WWWS(I,1)+ABS(WWWS(I,1)))*S1_S(I,1)-        &
	           (WWWS(I,2)-ABS(WWWS(I,2)))*S1_S(I,1))*(DTI/DTFA(I))/DZ(1)
     ...
     DO K=2,KBM1-1
       DO I=1,M
         ...
         TEMP=0.5*((WWWS(I,K+1)+ABS(WWWS(I,K+1)))*S1_S(I,K+1)-  &
	           (WWWS(I,K)-ABS(WWWS(I,K)))*S1_S(I,K-1))*(DTI/DTFA(I))/DZ(K)
         BETAIN(I,K)=(SMAX-S1_S(I,K))/(TEMP+1.E-10)

         TEMP=0.5*((WWWS(I,K)+ABS(WWWS(I,K)))*S1_S(I,K)-        &
	           (WWWS(I,K+1)-ABS(WWWS(I,K+1)))*S1_S(I,K))*(DTI/DTFA(I))/DZ(K)
     ...
     DO K=1,KBM1
       DO I=1,M
         IF(ISWETN(I)*ISWETNT(I) == 1) THEN
           ...
           END IF
           TEMP = 0.5_SP*TEMP
           S1_SF(I,K)=(S1_S(I,K)-TEMP*(DTI/DTFA(I))/DZ(K)) 
         END IF
       END DO
     END DO  !! SIGMA LOOP
...
!--------------------------------------------------------------------
   DO K=1,KBM1
     DO I=1,M
       IF(ISWETN(I)*ISWETNT(I) == 1) THEN
       IF(K == 1) THEN
         TEMP=-WTS(I,K+1)*(S1(I,K)*DZ(K+1)+S1(I,K+1)*DZ(K))/(DZ(K)+DZ(K+1))
       ELSE IF(K == KBM1) THEN
         TEMP= WTS(I,K)*(S1(I,K)*DZ(K-1)+S1(I,K-1)*DZ(K))/(DZ(K)+DZ(K-1))
       ELSE
         TEMP= WTS(I,K)*(S1(I,K)*DZ(K-1)+S1(I,K-1)*DZ(K))/(DZ(K)+DZ(K-1))-&
               WTS(I,K+1)*(S1(I,K)*DZ(K+1)+S1(I,K+1)*DZ(K))/(DZ(K)+DZ(K+1))
       END IF
