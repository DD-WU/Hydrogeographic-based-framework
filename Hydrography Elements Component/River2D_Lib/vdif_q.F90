...
!------------------------------------------------------------------------------!
   ...
   DO K=2,KBM1
     DO I=1,M
      IF(ISWETN(I) == 1)THEN
       BOYGR(I,K) = GEE * (RHO1(I,K-1)-RHO1(I,K))/(DZZ(K-1)*D(I))
...
!------------------------------------------------------------------------------!
   ...
   DO K=2,KBM1
     DO I=1,M
       IF(Z(K) > -0.5_SP)L(I,K) = MAX(L(I,K),KAPPA*L0(i)) 
...
!JQI   DTEF = Q2*SQRT(Q2)/(B1*Q2L + small)*STF
...
!------------------------------------------------------------------------------!
   ...
   DO  K = 2, KBM1
     DO  I = 1, M
       IF(ISWETN(I) == 1)THEN
         DTEF(I,K) = DTEF(I,K) * (1.+E2*((1./ABS(Z(K)-Z(1))+1./ &
                     ABS(Z(K)-Z(KB)))*L(I,K)/(D(I)*KAPPA))**2)
