...
!---------------------------------------------------------------------------------
   ...
   do i=1,m 
     ...
     do k=1,kbm1
       if(isonb(i) == 2) then
         xflux(i,k)= (vflux(k)-vflux(k+1))*art1(i)/dz(k)
       else
         xflux(i,k)=xflux(i,k)+ (vflux(k)-vflux(k+1))*art1(i)/dz(k)
       end if
     end do
     end if
   end do
  ...
  if(point_st_type == 'calculated') then
    if(inflow_type == 'node') then
        do j=1,numqbc
          ...
          fpoint=fdis(j)
          do k=1,kbm1
            xflux(jj,k)=xflux(jj,k) - qdis(j)*vqdist(j,k)*fpoint/dz(k)
          end do
        end do
...
!==============================================================================|
  ...
  Do i=1,m
    ...
    !Surface
    au(1) = 0.0
    cu(1)=      - deltat*(kh(i,2)+umol)/(dzz(1)*dz(1)*dsqrd)
    bu(1)=  1.0 - cu(1) 

    !Interior
    do k=2,kbm1-1
      au(k) =     - deltat*(kh(i,k  )+umol)/(dzz(k-1)*dz(k)*dsqrd)
      cu(k) =     - deltat*(kh(i,k+1)+umol)/(dzz(k  )*dz(k)*dsqrd)
      bu(k) = 1.0 - cu(k) - au(k) 
    end do

    !Bottom
     au(kbm1) =     - deltat*(kh(i,kbm1)+umol)/(dzz(kbm1-1)*dz(kbm1)*dsqrd)
     cu(kbm1) = 0.0
     bu(kbm1) = 1.0 - au(kbm1) 
...
!--------------------------------------------
  if(iobcn > 0) then
    do i=1,iobcn
      ...
      do k=1,kbm1
        f2d=f2d+f(j,k)*dz(k)
        f2d_next=f2d_next+fn(j1,k)*dz(k)
        xflux2d=xflux2d+fflux_obc(i,k)*dz(k)
      end do
