...
!==============================================================================|
   ...
   INTEGER NGL !ne               !!GLOBAL NUMBER OF ELEMENTS   
   INTEGER MGL !np               !!GLOBAL NUMBER OF NODES         
...
MODULE CONTROL
   ...
   CHARACTER(LEN=80) CASENAME1      !!LETTER ACRONYM SPECIFYING CASE IDENTITY (MAX 80 CHARS)
...
!--File Unit Specifiers (see iofiles.f for details)----------------------------!
   INTEGER  INRUN,INDEP,INGRD,INOBC,INCWH,INRIV,INITS,INOEL, &
            INJUL,INHFX,INWND,INELF,INUVF,INLAG,INSPO, &
...
!  HH: NEW OPEN BOUNDARY CONDITION
...
!---------------surface/bottom boundary conditions---------------------------------!
   ...
   REAL(SP), ALLOCATABLE :: BFWDIS(:)        !!GROUNDWATER FLUX AT CURRENT TIME
...
!----------------surface forcing: wind field----------------------------------------!
   ...
   Real(sp), allocatable  :: pre(:,:)     !!  surface pressure      zhangzhuo
...
!----------------boundary conditions: uniform meteo conditions----------------------!
   ...
   REAL(SP), ALLOCATABLE  :: DQEVAP(:)        !!EVAPORATION   
