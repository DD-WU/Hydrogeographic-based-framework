...
!---------------surface/bottom/edge boundary conditions-----------------------------!
   ...
   ALLOCATE(UUWIND(NT))          ;UUWIND = ZERO     !!SURFACE X-WIND 
   ALLOCATE(VVWIND(NT))          ;VVWIND = ZERO     !!SURFACE Y-WIND 
   ...
   ALLOCATE(VLCTYQ(NUMQBC))      ;VLCTYQ = ZERO     !!FLUX ANGLE 
          MEMCNT = NT*9*NDB + MT*2*NDB + NUMQBC*9*NDB + MEMCNT
