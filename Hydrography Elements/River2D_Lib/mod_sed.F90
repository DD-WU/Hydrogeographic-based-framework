...
!=======================================================================
  ...
  Call Get_Val(isedforce,sedfile,'SED_FORCE',line=linenum,echo=.false.)
...
!=======================================================================
  ...
  use wave, only :  wave_T, wave_h,wave_k
  ...
  real(sp) :: uvm, chezy,dd,k0,Ab, fw,  waveh
  real(sp)  concd(0:n)
  ...
  if(susload)then
    !  calculate sediment carrying force  
 do ised=1, nsed
   do i=1,n             ! element
	...
	dd=d1(i)
	if(d1(i)<=0.1) dd=0.1
	concd(i)=0.023*1000*2.65/1.65*uvm**3/Chezy**2/dd/(sed(ised)%Wset) *0.33333
    enddo
   do i=1,m
       Sed(ised)%force(I) = SUM(concd((NBVE(I,1:NTVE(I)))))/FLOAT(NTVE(I))
      if(iwave==1) then            !   +wave
	  if(wave_T(i)/=0) then
    	dd=d(i)
	   if(dd<=0.1) dd=0.1
     k0=0.001
   ...
   if(wave_h(i)>0.75*dd.and.dd<1.) waveh=0.75*dd           !  浅水区域对波高进行限制
   if(dd<=0.2) waveh=0.                                    !  是否限定最大泥沙浓度？？？


      Sed(ised)%force(I) =   Sed(ised)%force(i)              &
	  +0.004*1000*2.65/1.65*fw*waveh**3/(wave_T(i)**3*9.81*dd*sed(ised)%Wset*sinh(dd*wave_k(i))**3)
      endif
	  endif
 Sed(ised)%force(I)= min(Sed(ised)%force(I), 50.)         !  是否限定最大泥沙浓度？？？
    enddo   ! i
    enddo    ! ised
...
!=======================================================================
  ...
  do i=1,nsed
	...
	allocate(sed(i)%force(0:mt     )) ;     sed(i)%force = 0.0   
  end do
...
!==========================================================================
  ...
  implicit none
  integer  :: ised ,i,k
  real(sp) :: bed_por,bed_frac,tau_w,tau_ce,erate,dep,erosion,active, flux
...
!       用挟沙力方法算冲刷------------------------------------------------------
    flux = 0.
     do k=1,kbm1
	 if( Sed(ised)%conc(i,k)< Sed(ised)%force(I)) & !  冲刷发生
	  erosion =dz(k)*d(i)*abs(Sed(ised)%conc(i,k)-Sed(ised)%force(i))*0.5 + flux
     enddo
!-------------------------------------------------------------------------------
  ...
  do ised=1,nsed
    do i=1,m
   !   sed(ised)%conc(i,kbm1) = sed(ised)%conc(i,kbm1) + sed(ised)%eflx(i)/(d(i)*dz(kbm1))
    do k=1,kbm1
    if( Sed(ised)%conc(i,k)< Sed(ised)%force(I)) then !  冲刷发生
...
!  real(sp) ::  concd(0:n), uvm, chezy, dd, domga,ee,wavek,wavekn
  
  eps = epsilon(eps)

  !Loop over Sediment Class  
  if (.not.isedforce) then
...
!-------------------------------------------------------------------------------------
  ...
  !   DO k=1,kbm1
       if( Sed(ised)%conc(i,kbm1)> Sed(ised)%force(I)) then !  沉降发生
	   flux=Sed(ised)%wset*(Sed(ised)%conc(i,kbm1)-Sed(ised)%force(i))*dtsed
	   Sed(ised)%conc(i,kbm1)=(dz(kbm1)*dt(i)*Sed(ised)%conc(i,kbm1)-flux)/(d(i)*dz(kbm1))
	   Sed(ised)%conc(i,kbm1)=max(Sed(ised)%conc(i,kbm1),  0.)
       endif        
  !   END DO          !  k
