...
!  solve the arc length through the earth center
   ...
   AOB=ACOS(AOB)
   ARCL=REARTH*AOB
!   if(arcl.gt.rearth) then
!     write(203,'(9e15.6)') arcl,xb,xa,yb,ya,cos(y2),y2,yy2
!   endif
   RETURN
   END SUBROUTINE ARC
...
!c--------------------------------------------------------------------
!c      function:
!c           calculate the area of a triangle on a spherical plane
!c      input:
!c           side1,side2 and side3: are 3 arc lenth for one triangle
!c      output:
!c           areal: is area of a triangle on a spherical plane
!c--------------------------------------------------------------------
   IMPLICIT NONE
