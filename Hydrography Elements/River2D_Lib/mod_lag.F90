...
!----determine sigma layers above and below particle (for u/v velocities)--------!
     if(zp > zz(1))then     !!particle near surface
       k1  = 1
       k2  = 1
       zf1 = 1.0_sp 
       zf2 = 0.0_sp
     else if(zp < zz(kbm1)) then !!particle near bottom
       k1 = kbm1
       k2 = kbm1
       zf1 = 1.0_sp
       zf2 = 0.0_sp
...
!------------------------------------------------------------------------------|
   ...
   use all_vars, only : nv,aw0,awx,awy,xc,yc,kbm1,zz
   implicit none
...
!===============================================================================!
   ...
   do !loop over objects in list
     ...
     !bottom
     if(sigloc < zz(kbm1))then
       k1 = kbm1
       k2 = kbm1
       alpha = .5
     else !intermediate 
       do k=1,kbm1-1
         if(sigloc  < zz(k) .and. sigloc >= zz(k+1) )then
           k1 = k
           k2 = k+1 
           dsig = zz(k1)-zz(k2)
           alpha = (zz(k)-sigloc)/dsig
         endif
       end do
     endif
