...
!JQI<added for new obcs on 07/14/04
   USE MOD_OBCS
!>JQI 07/14/04
   USE MOD_EQUITIDE
...
!
     ...
     DO I=1,NBFWTIME
       ...
       IF(MSR)WRITE(IOPRT,5000) (RTEMP(J),J = 1,IBFW_GL) !CHARLIE FLAGG
     END DO
     DEALLOCATE(RTEMP)
...
!==============================================================================|
   ...
   IF(mean_flow==0.and.IBCN_GL(1) > 0)THEN
...
!
     ...
     NCNT = 0
    if(EQUI_TIDE==1) APT_FACT_EQUI = 0
     DO I=1,IBCN_GL(1)
    ...
    if(EQUI_TIDE==1)then
       ...
       IF(MSR)WRITE(IOPRT,*) (APT(I,J), J=1,6)
       IF(MSR)WRITE(IOPRT,*) (PHAI(I,J), J=1,6)
     END DO
...
!          call   chazhi            ! =====> elsbc(i,j)
!        READ(INJUL,*) (ELSBC(J,I),J=1,IBCN_GL(1))
      END DO
     END IF
...
!==============================================================================|
!    precipitation: m/year       "qprec"                                       |
!    evaporation:   m/year       "qevap"                                       |
!    wind:          wds (speed) wdd (direction)       			       |
!    heat flux:     w/m^2                              			       |
!         note: the code for precipitation/evaporation has not been tested-----|
!==============================================================================|
...
!
     ...
     ALLOCATE(DQPREC(NCNT),DQEVAP(NCNT))
     ...
     DO I=1,NCNT
       ...
       DQPREC(I) = QPREC / (86400.0_SP*365.0_SP)
       DQEVAP(I) = QEVAP / (86400.0_SP*365.0_SP)
