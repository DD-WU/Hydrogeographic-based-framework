...
!
   ...
   ALLOCATE(TTMP(IOBCN,KBM1));  TTMP = 0.0_SP
     DO I=1,IOBCN
       ...
       DO K=1,KBM1
         T2D=T2D+T1(J,K)*DZ(K)
         T2D_NEXT=T2D_NEXT+TF1(J1,K)*DZ(K)
...
!qxu{	 
         DO K=1,KBM1
           ...
           IF(K == 1)THEN
            TMAX = MAX(TMAX,(T1(J,K)*DZ(K+1)+T1(J,K+1)*DZ(K))/(DZ(K)+DZ(K+1)))
            TMIN = MIN(TMIN,(T1(J,K)*DZ(K+1)+T1(J,K+1)*DZ(K))/(DZ(K)+DZ(K+1)))
           ELSE IF(K == KBM1)THEN
            TMAX = MAX(TMAX,(T1(J,K)*DZ(K-1)+T1(J,K-1)*DZ(K))/(DZ(K)+DZ(K-1)))
            TMIN = MIN(TMIN,(T1(J,K)*DZ(K-1)+T1(J,K-1)*DZ(K))/(DZ(K)+DZ(K-1)))
           ELSE
            TMAX = MAX(TMAX,(T1(J,K)*DZ(K-1)+T1(J,K-1)*DZ(K))/(DZ(K)+DZ(K-1)), &
                   (T1(J,K)*DZ(K+1)+T1(J,K+1)*DZ(K))/(DZ(K)+DZ(K+1)))
            TMIN = MIN(TMIN,(T1(J,K)*DZ(K-1)+T1(J,K-1)*DZ(K))/(DZ(K)+DZ(K-1)), &
                   (T1(J,K)*DZ(K+1)+T1(J,K+1)*DZ(K))/(DZ(K)+DZ(K+1)))
           END IF
...
!qxu}	 
        ELSE
         DO K=1,KBM1
!           TF1(J,K) = T1(J,K)
!          GWC  ADD TS NUDGING ON OPEN BOUNDARY
!QXU           TF1(J,K) = T1(J,K) - ALPHA_OBC*RAMP_TS*(T1(J,K)-TEMP_OBC(I))
!          GWC
...
!
   ...
   ALLOCATE(STMP(IOBCN,KBM1));  STMP = 0.0_SP
     DO I=1,IOBCN
       ...
       DO K=1,KBM1
         S2D=S2D+S1(J,K)*DZ(K)
         S2D_NEXT=S2D_NEXT+SF1(J1,K)*DZ(K)
         XFLUX2D=XFLUX2D+XFLUX_OBC(I,K)*DZ(K)
       END DO
...
!QXU{	 
         DO K=1,KBM1
           ...
           IF(K == 1)THEN
            SMAX = MAX(SMAX,(S1(J,K)*DZ(K+1)+S1(J,K+1)*DZ(K))/(DZ(K)+DZ(K+1)))
            SMIN = MIN(SMIN,(S1(J,K)*DZ(K+1)+S1(J,K+1)*DZ(K))/(DZ(K)+DZ(K+1)))
           ELSE IF(K == KBM1)THEN
            SMAX = MAX(SMAX,(S1(J,K)*DZ(K-1)+S1(J,K-1)*DZ(K))/(DZ(K)+DZ(K-1)))
            SMIN = MIN(SMIN,(S1(J,K)*DZ(K-1)+S1(J,K-1)*DZ(K))/(DZ(K)+DZ(K-1)))
           ELSE
            SMAX = MAX(SMAX,(S1(J,K)*DZ(K-1)+S1(J,K-1)*DZ(K))/(DZ(K)+DZ(K-1)), &
                   (S1(J,K)*DZ(K+1)+S1(J,K+1)*DZ(K))/(DZ(K)+DZ(K+1)))
            SMIN = MIN(SMIN,(S1(J,K)*DZ(K-1)+S1(J,K-1)*DZ(K))/(DZ(K)+DZ(K-1)), &
                   (S1(J,K)*DZ(K+1)+S1(J,K+1)*DZ(K))/(DZ(K)+DZ(K+1)))
           END IF
...
!           SF1(J,K) = S1(J,K)
!          GWC  ADD TS NUDGING ON OPEN BOUNDARY
!QXU           SF1(J,K) = S1(J,K) - ALPHA_OBC*RAMP_TS*(S1(J,K)-SALT_OBC(I))
!          GWC
