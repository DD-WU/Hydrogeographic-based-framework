...
void Region::update()
{
    ...
    if (parameter->Statesptr->BuildingFlag==0)FloodplainQ(parameter->Statesptr, parameter->Parptr, parameter->Solverptr, parameter->Arrptr);//这个地方不太好，应该加个建筑物flag栅格，然后内部判断是建筑物还是地块，洪敏看到的话改吧
